...
using DILogic = КВАНТ.Logical.ProgrammEngine.array_l_element.DI;

namespace КВАНТ.ViewsFormsPages.Pages.VisualScreen.Controls
{
    /// <summary>
    /// Interaction logic for DI.xaml
    /// </summary>
    public partial class DI : UserControl, IControlVisual
    {
        ...
        private DILogic self_element;

        public DI(object element, Point posPoint, bool b)
        {
            InitializeComponent();
            self_element = (DILogic)element;
            DataContext = self_element.Content;
            VElement = new VisualElement { X = self_element.X, Y = self_element.Y };
            Position = posPoint;
            if (b)
            {
